...
if (
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $postData = $_POST;
        if (isset($postData['userId']) && !empty($postData['userId']) && isset($postData['idCampaign']) && !empty($postData['idCampaign'])) {

            $sql = "SELECT SUM(NumberPrayers) AS TotalPrayers FROM SchedulePrayers WHERE UserIdP = '$postData[userId]' AND 	campaign_id  = '$postData[idCampaign]'";
